...
export default function PowerGraph({ telemetry, currentT, peakHp }: PowerGraphProps) {
  if (telemetry.length === 0) return null;

  const totalDuration = telemetry[telemetry.length - 1].t;
  const maxHp = niceMax(peakHp * 1.08);
  const plotWidth = WIDTH - PAD_LEFT - PAD_RIGHT;
  const plotHeight = HEIGHT - PAD_TOP - PAD_BOTTOM;

  const xFor = (t: number) => PAD_LEFT + (t / totalDuration) * plotWidth;
  const yFor = (hp: number) =>
    PAD_TOP + plotHeight - (Math.max(0, hp) / maxHp) * plotHeight;
  ...
  const linePath = points
    .map(
      (s, i) => `${i === 0 ? "M" : "L"} ${xFor(s.t).toFixed(1)} ${yFor(s.hp).toFixed(1)}`,
    )
    .join(" ");
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <path d={areaPath} fill={POWER_COLOR} opacity={0.1} />
        <path
          d={linePath}
          fill="none"
          stroke={POWER_COLOR}
        ...
        <circle cx={xFor(last.t)} cy={yFor(last.hp)} r={6} fill={SURFACE_COLOR} />
        <circle cx={xFor(last.t)} cy={yFor(last.hp)} r={4} fill={POWER_COLOR} />
        <text
          x={Math.min(xFor(last.t) + 8, WIDTH - PAD_RIGHT - 28)}
          y={Math.max(yFor(last.hp) - 8, PAD_TOP + 8)}
          fontSize={12}
